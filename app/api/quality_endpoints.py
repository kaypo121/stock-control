...
def _build_response(a: QualityAssessment) -> QualityAssessmentResponse:
    ...
    produce_resp   = ProduceDetailResponse.model_validate(a.produce_detail)   if a.produce_detail   else None
    livestock_resp = LivestockDetailResponse.model_validate(a.livestock_detail) if a.livestock_detail else None
    poultry_resp   = PoultryDetailResponse.model_validate(a.poultry_detail)   if a.poultry_detail   else None
    fish_resp      = FishDetailResponse.model_validate(a.fish_detail)         if a.fish_detail      else None
...
def list_assessments(
    ...
    category:       Optional[str] = Query(None, description="Filter by category (e.g. Crop, Fish)"),
    farmer_id:      Optional[int] = Query(None, description="Filter by farmer ID"),
    recommendation: Optional[str] = Query(None, description="Filter by recommendation: Buy | Buy with Caution | Do Not Buy"),
...
def quality_summary(
    ...
    rows = q.group_by(
        QualityAssessment.product_name,
        QualityAssessment.category,
    ).order_by(func.avg(QualityAssessment.quality_score).desc()).all()
    ...
    for row in rows:
        ...
        grades = db.query(
            ...
            QualityAssessment.product_name == row.product_name
        ).group_by(QualityAssessment.grade_classification).all()
        ...
        recs = db.query(
            ...
            QualityAssessment.product_name == row.product_name
        ).group_by(QualityAssessment.purchase_recommendation).all()
