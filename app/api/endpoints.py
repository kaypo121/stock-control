    ...
    for b in balances:
        if b.current_stock <= b.reorder_level:
            low_stock_list.append(
                stock_schemas.BalanceSummary(
                    farmer_name=b.farmer.full_name,
                    product_name=b.product.product_name,
                    warehouse_name=(
                        b.warehouse.warehouse_name if b.warehouse else None
                    ),
...
def register_farmer(farmer: stock_schemas.FarmerCreate, db: Session = Depends(get_db)):
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail="Farmer with this name already registered."
        )
...
def register_product(
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail="Product with this name already registered.",
        )
...
def register_warehouse(
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail="Warehouse with this name already registered.",
        )
