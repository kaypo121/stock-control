...
from app.models.stock_models import StockTransaction, StockBalance
...
class ForecastService:
    ...
    def generate_baseline_forecast(self, product_id: int) -> Dict[str, Any]:
        ...
        balances = self.db.query(StockBalance).filter(StockBalance.product_id == product_id).all()
        ...
        recommendation = "Stock levels are currently stable based on consumption trends."
        ...
        if total_current <= total_reorder:
            ...
            recommendation = "Stock is projected to run out within 30 days. Plan reordering soon."
