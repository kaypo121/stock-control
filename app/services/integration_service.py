...
from app.config import DATA_ERROR_LOGS_DIR, DATA_PROCESSED_DIR
from app.models.stock_models import (
    Farmer, Product, Warehouse, StockTransaction, StockBalance, StockAlert, ImportLog
)
from app.models.integration_models import (
    DataIntegrationSession, IntegrationErrorRecord, IntegrationReport
)
from app.models.quality_models import QualityAssessment
from app.schemas.integration_schemas import (
    AnalysisSummary, CleaningSummary, MappingSummary,
    IntegrationSummary, InventoryStats, ValidationResult,
    DataImportReport, StockSummaryReport, ProductAvailabilityReport,
    InventoryHealthReport, LowStockAlertReport, IntegrationPipelineResult,
)
...
COLUMN_ALIASES: Dict[str, str] = {
    # Farmer
    "farmer": "farmer_name", "farmer_name": "farmer_name",
    "supplier": "farmer_name", "supplier_name": "farmer_name",
    "grower": "farmer_name", "producer": "farmer_name",
    "farmer_id": "farmer_id", "supplier_id": "farmer_id",

    # Product
    "product": "product_name", "product_name": "product_name",
    "crop": "product_name", "commodity": "product_name",
    "item": "product_name", "goods": "product_name",
    "species": "product_name", "livestock": "product_name",
    "fish_species": "product_name",

    # Category
    "category": "category", "type": "category",
    "product_type": "category", "item_type": "category",

    # Quantity / weight
    "quantity": "quantity", "qty": "quantity",
    "amount": "quantity", "volume": "quantity",
    "weight": "quantity", "weight_kg": "quantity",
    "production": "quantity", "production_mt": "quantity",
    "harvest": "quantity",

    # Unit
    "unit": "unit", "units": "unit",
    "uom": "unit", "measure": "unit",

    # Transaction type
    "transaction_type": "transaction_type", "type": "transaction_type",
    "movement_type": "transaction_type", "action": "transaction_type",

    # Date
    "date": "transaction_date", "transaction_date": "transaction_date",
    "harvest_date": "transaction_date", "purchase_date": "transaction_date",
    "sale_date": "transaction_date", "market_day": "transaction_date",
    "year": "transaction_date",

    # Warehouse / Location
    "warehouse": "warehouse_name", "warehouse_name": "warehouse_name",
    "storage": "warehouse_name", "location": "warehouse_name",
    "market": "warehouse_name", "store": "warehouse_name",
    ...
    "cost": "unit_price", "rate": "unit_price",

    # Reference
    "note": "reference_note", "notes": "reference_note",
    "reference": "reference_note", "reference_note": "reference_note",
    "batch": "batch_number", "batch_number": "batch_number",
    "lot": "batch_number",

    # Livestock / Poultry / Fish specific
    "breed": "breed", "age": "age",
    "age_months": "age", "age_weeks": "age",
    "health_status": "health_status", "health": "health_status",
    "body_condition_score": "body_condition_score", "bcs": "body_condition_score",
    "vaccination": "vaccination_status", "vaccination_status": "vaccination_status",

    # Quality
    "freshness_score": "freshness_score", "freshness": "freshness_score",
    "ripeness": "ripeness_level", "ripeness_level": "ripeness_level",
    "moisture": "moisture_content_pct", "moisture_pct": "moisture_content_pct",
    "damage_pct": "visible_damage_pct", "visible_damage": "visible_damage_pct",
    "pest_damage": "pest_damage_pct", "disease": "disease_symptoms",

    # Inventory
    "opening_stock": "opening_stock", "opening_balance": "opening_stock",
...
SCHEMA_SIGNATURES: Dict[str, List[str]] = {
    "PRODUCTION_ESTIMATES":   ["crop", "production", "region", "district", "year"],
    "WHOLESALE_PRICES":       ["commodity", "price", "year"],
    "RAINFALL":               ["region", "year", "rainfall"],
    "SYNTHETIC_TRANSACTIONS": ["farmer_name", "product_name", "quantity", "unit", "transaction_type", "transaction_date"],
    "INVENTORY_RECORDS":      ["product_name", "quantity", "unit", "opening_stock", "warehouse_name"],
    "SALES_RECORDS":          ["product_name", "quantity", "unit", "sale_date"],
    "PURCHASE_RECORDS":       ["product_name", "quantity", "unit", "purchase_date", "supplier"],
    ...
    "QUALITY_ASSESSMENTS":    ["product_name", "batch_number", "freshness_score", "quality_score"],
...
VALID_TX_TYPES = {"STOCK_IN", "STOCK_OUT", "DAMAGE", "RETURN", "TRANSFER", "ADJUSTMENT"}
...
def _get_or_create_farmer(db: Session, name: str, region: Optional[str],
                           district: Optional[str],
                           created_counter: List[int],
                           matched_counter: List[int]) -> Optional[Farmer]:
    name_c = str(name).strip()
    if not name_c:
        return None
    # Exact match (case-insensitive)
    farmer = db.query(Farmer).filter(
        func.lower(Farmer.full_name) == name_c.lower()
    ).first()
    if farmer:
        matched_counter[0] += 1
        return farmer
...
def _get_or_create_product(db: Session, name: str, category: str, unit: str,
                            created_counter: List[int],
                            matched_counter: List[int]) -> Optional[Product]:
    name_c = str(name).strip()
    if not name_c:
        return None
    product = db.query(Product).filter(
        func.lower(Product.product_name) == name_c.lower()
    ).first()
    if product:
        matched_counter[0] += 1
        return product
    ...
    for p in all_products:
        r = difflib.SequenceMatcher(None, p.product_name.lower(), name_c.lower()).ratio()
        if r > best_score:
            best_score = r
            best = p
    if best and best_score >= 0.92:
...
def _get_or_create_warehouse(db: Session, name: str, region: Optional[str],
                              district: Optional[str],
                              created_counter: List[int],
                              matched_counter: List[int]) -> Optional[Warehouse]:
    name_c = str(name).strip()
    if not name_c:
        return None
    wh = db.query(Warehouse).filter(
        func.lower(Warehouse.warehouse_name) == name_c.lower()
    ).first()
    if wh:
        matched_counter[0] += 1
        return wh
...
def _update_balance(db: Session, farmer_id: int, product_id: int,
                    warehouse_id: Optional[int], tx_type: str,
                    qty_base: float, balance_cache: Dict) -> None:
...
def _compute_inventory_stats(db: Session) -> List[InventoryStats]:
    ...
    for product in products:
        ...
        total_in   = sum(t.quantity for t in txs if t.transaction_type in ("STOCK_IN", "RETURN"))
        total_out  = sum(t.quantity for t in txs if t.transaction_type in ("STOCK_OUT", "TRANSFER"))
...
def _validate(db: Session, session: DataIntegrationSession) -> ValidationResult:
    ...
    if neg_balances:
        issues.append(f"{neg_balances} stock balance record(s) have negative current_stock.")

    # Orphaned transactions (no farmer / no product)
    orphaned = db.query(StockTransaction).filter(
        StockTransaction.farmer_id == None
    ).count()
    if orphaned:
    ...
    if wh_mismatches:
        issues.append(f"{wh_mismatches} transaction(s) reference non-existent warehouse IDs.")

    # Quantity anomalies: extreme outliers (> 3 standard deviations)
    result = db.execute(text(
        "SELECT AVG(quantity), MAX(quantity) FROM stock_transactions"
    )).fetchone()
    qty_anomalies = 0
    if result and result[0]:
        ...
        if max_qty > avg_qty * 100:
            qty_anomalies += 1
            issues.append(f"Quantity outlier detected: max={max_qty:,.0f} vs avg={avg_qty:,.0f}")
...
def _recommended_actions(inv_stats: List[InventoryStats], validation: ValidationResult) -> List[str]:
    actions = []
    ...
    for s in oos:
        actions.append(f"URGENT: {s.product_name} is OUT OF STOCK — place purchase order immediately.")
    for s in critical:
        actions.append(f"CRITICAL: {s.product_name} stock ({s.total_current_stock:,.0f} {s.unit}) is critically low — reorder at once.")
    for s in low:
        actions.append(f"LOW STOCK: {s.product_name} ({s.total_current_stock:,.0f} {s.unit}) — schedule reorder soon.")

    high_turnover = [s for s in inv_stats if s.stock_turnover_rate > 2.0]
    for s in high_turnover:
        actions.append(f"HIGH TURNOVER: {s.product_name} has rapid stock movement (rate {s.stock_turnover_rate:.2f}) — increase buffer stock.")
...
def _generate_reports(
...
) -> Tuple[DataImportReport, StockSummaryReport, ProductAvailabilityReport,
           InventoryHealthReport, LowStockAlertReport]:
    ...
    available    = [{"product": s.product_name, "stock": s.available_stock, "unit": s.unit} for s in inv_stats if s.status == "HEALTHY"]
    oos_list     = [{"product": s.product_name, "stock": 0, "unit": s.unit} for s in inv_stats if s.status == "OUT_OF_STOCK"]
    low_list     = [{"product": s.product_name, "stock": s.available_stock, "unit": s.unit, "reorder_level": s.reorder_level} for s in inv_stats if s.status in ("LOW_STOCK", "CRITICAL")]
    ...
    health_counts = {"HEALTHY": 0, "LOW_STOCK": 0, "CRITICAL": 0, "OUT_OF_STOCK": 0}
    for s in inv_stats:
    ...
    low_items = [s for s in inv_stats if s.status in ("LOW_STOCK", "CRITICAL", "OUT_OF_STOCK")]
    low_alerts = []
    for s in low_items:
        low_alerts.append({
    ...
    return import_report, stock_report, avail_report, health_report, alert_report
...
class DataIntegrationService:
    ...
    def run_pipeline(self, file_path: Path, initiated_by: str = "system") -> IntegrationPipelineResult:
        """
    ...
    def _execute_pipeline(
        ...
        sample_issues = []
        for col in raw_cols:
            if col in ("quantity", "weight_kg", "production", "production_mt", "price"):
                non_numeric = df[col].apply(
                    lambda v: v is not None and _parse_number(v) is None
                ).sum()
                if non_numeric:
                    invalid_count += int(non_numeric)
                    sample_issues.append(f"Column '{col}': {non_numeric} non-numeric value(s)")
        ...
        for col, default in fill_defaults.items():
            if col in df.columns:
                ...
                nulls_filled += filled
                if filled:
                    cleaning_actions.append(f"Filled {filled} missing '{col}' with default '{default}'.")

        # Standardise units
        units_std = 0
        if "unit" in df.columns:
            original_units = df["unit"].copy()
            df["unit"] = df["unit"].apply(lambda v: _normalise_unit(str(v)) if v else "kg")
        ...
        if "transaction_type" in df.columns:
            ...
            corrections = int((df["transaction_type"] != original_types.str.upper().str.strip()).sum())
            format_corrections += corrections
            if corrections:
                cleaning_actions.append(f"Corrected {corrections} invalid transaction_type value(s) to 'STOCK_IN'.")
        ...
        for idx, row in df.iterrows():
            ...
            try:
                row_result = self._process_row(
                    row_dict, idx, session, file_path.name, detected_schema,
                    farmers_created, farmers_matched,
                    products_created, products_matched,
                    wh_created, wh_matched,
                    balance_cache,
                )
                ...
                self.db.add(IntegrationErrorRecord(
                    session_id   = session.session_id,
                    row_index    = int(idx),
                    step         = "STEP4",
        ...
        status = "SUCCESS" if not errors else ("PARTIAL" if tx_inserted > 0 else "FAILED")
        session.status       = status
        session.completed_at = datetime.utcnow()
        self.db.commit()

        (import_report, stock_report, avail_report,
         health_report, alert_report) = _generate_reports(
            self.db, session, analysis, cleaning, mapping,
    ...
    def _process_row(
        self, row: Dict, idx: int, session: DataIntegrationSession,
        file_name: str, schema: str,
        farmers_created, farmers_matched,
        products_created, products_matched,
        wh_created, wh_matched,
        balance_cache: Dict,
        ...
        tx_date = _parse_date(date_raw) or datetime.utcnow()
        ...
        farmer = _get_or_create_farmer(
            self.db, farmer_name, region, district,
            farmers_created, farmers_matched
        )
        product = _get_or_create_product(
            self.db, product_name, category, unit_raw,
            products_created, products_matched
        )
        ...
        if wh_name:
            wh = _get_or_create_warehouse(
                self.db, wh_name, region, district,
                wh_created, wh_matched
            )
        ...
        # Update balance
        _update_balance(
            self.db, farmer.farmer_id, product.product_id,
            warehouse_id, tx_type, qty_base, balance_cache
        )
    ...
    def _read_file(self, file_path: Path) -> Tuple[Optional[pd.DataFrame], Optional[str]]:
    ...
    def _reshape_for_schema(self, df: pd.DataFrame, schema: str, file_name: str) -> pd.DataFrame:
        ...
        if schema == "PRODUCTION_ESTIMATES":
            ...
            if district_col:
                df["warehouse_name"] = df[district_col].apply(
                    lambda d: f"{str(d).strip().title()} District Storage" if d else "National Storage"
                )
    ...
    def _infer_category(self, product_name: Optional[str], schema: str) -> str:
        ...
        if any(x in p for x in ["maize", "rice", "sorghum", "millet", "wheat", "grain", "cereal"]):
            return "Grains"
        if any(x in p for x in ["yam", "cassava", "cocoyam", "tuber", "root"]):
            return "Roots & Tubers"
        if any(x in p for x in ["tomato", "pepper", "onion", "okra", "cabbage", "lettuce", "vegetable"]):
            return "Vegetables"
        if any(x in p for x in ["mango", "pineapple", "banana", "orange", "pawpaw", "fruit"]):
            return "Fruits"
        if any(x in p for x in ["cattle", "cow", "bull", "goat", "sheep", "pig", "livestock"]):
            return "Livestock"
        if any(x in p for x in ["chicken", "broiler", "layer", "turkey", "duck", "guinea fowl", "poultry"]):
            return "Poultry"
        if any(x in p for x in ["tilapia", "catfish", "tuna", "fish", "herring", "mackerel"]):
            return "Fish"
    ...
    def _infer_data_source(self, file_path: Path, schema: str) -> str:
        ...
        if "production" in name:  return "MoFA Production Estimates"
        if "price" in name:       return "MoFA Price Data"
        if "rainfall" in name:    return "Ghana Met Office"
        if "synthetic" in name:   return "Synthetic Dataset"
        if "aquaculture" in name: return "Ghana Fisheries Commission"
        return f"Manual Upload ({schema})"

    def _infer_target_tables(self, schema: str) -> List[str]:
        base = ["farmers", "products", "warehouses", "stock_transactions", "stock_balances"]
        if schema in ("LIVESTOCK_RECORDS", "POULTRY_RECORDS", "FISH_RECORDS",
                      "QUALITY_ASSESSMENTS"):
    ...
    def list_sessions(self, skip: int = 0, limit: int = 50) -> List[DataIntegrationSession]:
        return self.db.query(DataIntegrationSession).order_by(
            DataIntegrationSession.started_at.desc()
        ).offset(skip).limit(limit).all()

    def get_session(self, session_id: int) -> Optional[DataIntegrationSession]:
        return self.db.query(DataIntegrationSession).filter(
            DataIntegrationSession.session_id == session_id
        ).first()

    def get_session_reports(self, session_id: int) -> List[IntegrationReport]:
        return self.db.query(IntegrationReport).filter(
            IntegrationReport.session_id == session_id
        ).all()

    def get_report(self, report_id: int) -> Optional[IntegrationReport]:
        return self.db.query(IntegrationReport).filter(
            IntegrationReport.report_id == report_id
        ).first()
    ...
    def get_standalone_reports(self) -> Dict[str, Any]:
        ...
        inv_stats  = _compute_inventory_stats(self.db)
        validation = ValidationResult(
            passed=True, duplicate_ids_found=0,
            constraint_violations=0, warehouse_mismatches=0,
            quantity_anomalies=0, issues=["Standalone report — no import session."]
        )
        actions = _recommended_actions(inv_stats, validation)
        now = datetime.utcnow()
        ...
        stock_report = StockSummaryReport(
            generated_at=now, total_products=total_p, total_farmers=total_f,
            total_warehouses=total_w, total_transactions=total_t, inventory=inv_stats
        )
        available   = [{"product": s.product_name, "stock": s.available_stock, "unit": s.unit} for s in inv_stats if s.status == "HEALTHY"]
        oos_list    = [{"product": s.product_name, "stock": 0, "unit": s.unit} for s in inv_stats if s.status == "OUT_OF_STOCK"]
        low_list    = [{"product": s.product_name, "stock": s.available_stock, "unit": s.unit, "reorder_level": s.reorder_level} for s in inv_stats if s.status in ("LOW_STOCK", "CRITICAL")]
        avail_report = ProductAvailabilityReport(generated_at=now, available_products=available, out_of_stock=oos_list, low_stock=low_list)
        ...
        health_report = InventoryHealthReport(generated_at=now, healthy_count=hc["HEALTHY"], low_stock_count=hc["LOW_STOCK"], critical_count=hc["CRITICAL"], out_of_stock_count=hc["OUT_OF_STOCK"], overall_health_pct=overall, items=inv_stats)

        low_items = [s for s in inv_stats if s.status in ("LOW_STOCK", "CRITICAL", "OUT_OF_STOCK")]
        alert_report = LowStockAlertReport(
            generated_at=now, alert_count=len(low_items),
            critical_count=hc["CRITICAL"] + hc["OUT_OF_STOCK"],
            alerts=[{"product": s.product_name, "current_stock": s.total_current_stock, "reorder_level": s.reorder_level, "status": s.status, "unit": s.unit} for s in low_items],
            recommended_actions=actions
        )
    ...
    def scan_and_run_all(self, directory: Path, initiated_by: str = "system") -> List[IntegrationPipelineResult]:
