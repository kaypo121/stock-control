...
class ProduceAttributesInput(BaseModel):
    ...
    diameter_cm: Optional[float] = Field(
        None, ge=0, description="Diameter in centimetres (if applicable)"
    )
    color_quality: Optional[str] = Field(
        None,
        description="Color description (e.g. 'Bright Red', 'Pale Yellow')",
    )
    freshness_score: Optional[float] = Field(
        None, ge=0, le=10, description="Freshness score out of 10"
    )
    ripeness_level: Optional[RipenessLevel] = Field(None)
    moisture_content_pct: Optional[float] = Field(
        None, ge=0, le=100, description="Moisture content %"
    )
    visible_damage_pct: Optional[float] = Field(
        None, ge=0, le=100, description="% of surface with visible damage"
    )
    pest_damage_pct: Optional[float] = Field(
        None, ge=0, le=100, description="% showing pest damage"
    )
    disease_symptoms: Optional[str] = Field(
        None, description="Describe disease symptoms or 'None'"
    )
    estimated_shelf_life_days: Optional[int] = Field(
        None, ge=0, description="Estimated shelf life in days"
    )
...
class LivestockAttributesInput(BaseModel):
    ...
    species: Optional[str] = Field(
        None, description="Species (e.g. Cattle, Goat, Sheep, Pig)"
    )
    ...
    height_cm: Optional[float] = Field(
        None, ge=0, description="Height at withers in cm"
    )
    length_cm: Optional[float] = Field(None, ge=0, description="Body length in cm")
    body_condition_score: Optional[float] = Field(
        None, ge=1, le=5, description="BCS (1=emaciated, 5=obese)"
    )
    health_status: Optional[HealthStatus] = Field(None)
    vaccination_status: Optional[str] = Field(
        None, description="e.g. 'Up to date', 'Partial', 'None'"
    )
    disease_indicators: Optional[str] = Field(
        None, description="Describe or 'None observed'"
    )
    mobility_assessment: Optional[str] = Field(
        None, description="e.g. 'Normal', 'Slight limp', 'Lame'"
    )
    feeding_condition: Optional[str] = Field(
        None, description="e.g. 'Well-fed', 'Thin', 'Emaciated'"
    )
    reproductive_status: Optional[str] = Field(
        None, description="e.g. 'Pregnant', 'Lactating', 'Dry', 'N/A'"
    )
...
class PoultryAttributesInput(BaseModel):
    ...
    species: Optional[str] = Field(
        None, description="Species (e.g. Chicken, Turkey, Duck, Guinea Fowl)"
    )
    breed: Optional[str] = Field(
        None, description="Breed (e.g. Broiler, Layer, Kuroiler)"
    )
    ...
    egg_production_rate_pct: Optional[float] = Field(
        None, ge=0, le=100, description="Egg production rate % (layers only)"
    )
    feather_condition: Optional[str] = Field(
        None, description="e.g. 'Full, Glossy', 'Patchy', 'Bare patches'"
    )
    disease_signs: Optional[str] = Field(
        None, description="Describe or 'None observed'"
    )
...
class FishAttributesInput(BaseModel):
    ...
    species: Optional[str] = Field(
        None, description="Species (e.g. Tilapia, Catfish, Tuna, Herring)"
    )
    weight_kg: Optional[float] = Field(None, ge=0, description="Weight in kg")
    length_cm: Optional[float] = Field(None, ge=0, description="Length in cm")
    freshness_level: Optional[FreshnessLevel] = Field(None)
    eye_clarity: Optional[str] = Field(
        None, description="e.g. 'Clear, Bright', 'Slightly Cloudy', 'Sunken'"
    )
    gill_condition: Optional[str] = Field(
        None, description="e.g. 'Bright Red', 'Pink', 'Brown/Grey'"
    )
    odor_assessment: Optional[str] = Field(
        None, description="e.g. 'Fresh Sea Smell', 'Mild Odor', 'Foul'"
    )
    flesh_quality: Optional[str] = Field(
        None, description="e.g. 'Firm, Elastic', 'Soft', 'Mushy'"
    )
...
class QualityAssessmentRequest(BaseModel):
    ...
    warehouse_id: Optional[int] = Field(
        None, description="Warehouse where item is located"
    )
    ...
    def check_attributes_match_category(self) -> "QualityAssessmentRequest":
        ...
        if cat in (
            ...
            if self.livestock_attributes is None:
                raise ValueError(
                    "'livestock_attributes' is required for category 'Livestock'"
                )
        elif cat == ProductCategory.POULTRY:
            if self.poultry_attributes is None:
                raise ValueError(
                    "'poultry_attributes' is required for category 'Poultry'"
                )
...
class QualityScoring(BaseModel):
    ...
    market_readiness_score: float = Field(
        ..., description="Market readiness score 0–100"
    )
    estimated_market_value_ghs: Optional[float] = Field(
        None, description="Estimated value in Ghana Cedis"
    )
    risk_level: str = Field(..., description="Low / Medium / High")
    grade_classification: str = Field(
        ..., description="Premium / Grade A / Grade B / Grade C / Reject"
    )
